...
class Gallery_model extends Model {
  ...
  public function create_album($album_name, $id_users) {
    try {
      $this->check_album_name($album_name);
      $statement = $this->db->prepare("insert into albums(album_name, id_users) values (:album_name, :id_users)"); 
      $statement->execute(['album_name' => $album_name, 'id_users' => $id_users]); 
  ...
  public function photos($album_id) {
    try {
      $statement = $this->db->prepare("select photo_id, photo_name, id_users from photos where album_id = :album_id");
      $statement->execute(['album_id' => $album_id]);
      return $statement->fetchAll();
    } catch (PDOException $e) {
      throw new Exception(self::str_error_database);
  ...
  public function add_photo($album_id, $photo_name, $tmp_file, $id_users) {
    try {
      $this->check_photo_name($photo_name);
      $statement = $this->db->prepare("insert into photos(album_id, photo_name, fullsize, thumbnail, id_users) 
                                              values (:album_id, :photo_name, :fullsize, :thumbnail, :id_users)");
      $statement->execute(['album_id'=> $album_id, 
  ...
  public function photo($photo_id) {
    try {
      $statement = $this->db->prepare("select photo_id, photo_name, id_users from photos where photo_id = :photo_id");
